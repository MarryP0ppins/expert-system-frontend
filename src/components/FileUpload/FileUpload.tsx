...
const FileUpload: React.FC<InputProps> = ({ className, onChange, onDeleteClick, initialImageUrl, ...props }) => {
  ...
  return (
    <div className={cnFileUpload({ uploaded: !!imagePreview || initImage }) + ` ${className}`}>
      ...
      <label className={cnFileUpload('input-wrapper')} ref={labelInput}>
        <input {...props} type="file" className={cnFileUpload('input')} onChange={handleOnChange} />
      </label>
